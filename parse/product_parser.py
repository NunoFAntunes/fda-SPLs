...
class ProductParser(BaseParser):
    ...
    def _extract_product_code(self, product_element: ET.Element) -> Optional[CodedConcept]:
        ...
        code_element = XMLUtils.find_element(product_element, "code")
    ...
    def _extract_product_name(self, product_element: ET.Element) -> tuple[Optional[str], Optional[str]]:
        """Extract product name and suffix."""
        name_element = XMLUtils.find_element(product_element, "name")
        ...
        suffix_element = XMLUtils.find_element(name_element, "suffix")
    ...
    def _extract_form_code(self, product_element: ET.Element) -> Optional[CodedConcept]:
        ...
        form_code_element = XMLUtils.find_element(product_element, "formCode")
    ...
    def _extract_generic_name(self, product_element: ET.Element) -> Optional[str]:
        ...
        generic_element = XMLUtils.find_element(product_element, "asEntityWithGeneric")
        if generic_element is None:
            return None
        
        generic_medicine = XMLUtils.find_element(generic_element, "genericMedicine")
        if generic_medicine is None:
            return None
        
        name_element = XMLUtils.find_element(generic_medicine, "name")
    ...
    def _extract_package_info(self, product_element: ET.Element) -> Optional[PackageInfo]:
        """Extract packaging information."""
        content_element = XMLUtils.find_element(product_element, "asContent")
        ...
        # Extract quantity
        quantity_element = XMLUtils.find_element(content_element, "quantity")
        if quantity_element is not None:
            package_info.quantity = self._parse_quantity(quantity_element)
        
        # Extract container information
        container_element = XMLUtils.find_element(content_element, "containerPackagedProduct")
        if container_element is not None:
            # Extract container code
            code_element = XMLUtils.find_element(container_element, "code")
            if code_element is not None:
                package_info.code = XMLUtils.parse_coded_concept(code_element)
            
            # Extract container form code
            form_code_element = XMLUtils.find_element(container_element, "formCode")
            if form_code_element is not None:
    ...
    def _extract_marketing_info(self, manufactured_product_element: ET.Element) -> Optional[MarketingInfo]:
        """Extract marketing information."""
        subject_of_elements = XMLUtils.find_all_elements(manufactured_product_element, "subjectOf")
        
        for subject_of in subject_of_elements:
            marketing_act = XMLUtils.find_element(subject_of, "marketingAct")
            if marketing_act is not None:
                marketing_info = MarketingInfo()
                
                # Extract marketing code
                code_element = XMLUtils.find_element(marketing_act, "code")
                if code_element is not None:
                    marketing_info.marketing_code = XMLUtils.parse_coded_concept(code_element)
                
                # Extract status code
                status_element = XMLUtils.find_element(marketing_act, "statusCode")
                if status_element is not None:
                    marketing_info.status_code = XMLUtils.get_attribute(status_element, "code")
                
                # Extract effective time
                effective_time = XMLUtils.find_element(marketing_act, "effectiveTime")
                if effective_time is not None:
                    low_element = XMLUtils.find_element(effective_time, "low")
                    high_element = XMLUtils.find_element(effective_time, "high")
    ...
    def _extract_approval_info(self, manufactured_product_element: ET.Element) -> Optional[ApprovalInfo]:
        ...
        for subject_of in subject_of_elements:
            approval_element = XMLUtils.find_element(subject_of, "approval")
            if approval_element is not None:
                approval_info = ApprovalInfo()
                
                # Extract approval ID
                id_element = XMLUtils.find_element(approval_element, "id")
                if id_element is not None:
                    approval_info.approval_id = XMLUtils.get_attribute(id_element, "extension")
                
                # Extract approval type code
                code_element = XMLUtils.find_element(approval_element, "code")
                if code_element is not None:
                    approval_info.approval_type = XMLUtils.parse_coded_concept(code_element)
                
                # Extract territorial authority
                author_element = XMLUtils.find_element(approval_element, "author")
                if author_element is not None:
                    territorial_auth = XMLUtils.find_element(author_element, "territorialAuthority")
                    if territorial_auth is not None:
                        territory = XMLUtils.find_element(territorial_auth, "territory")
                        if territory is not None:
                            code_element = XMLUtils.find_element(territory, "code")
                            if code_element is not None:
    ...
    def _extract_routes(self, manufactured_product_element: ET.Element) -> List[RouteOfAdministration]:
        """Extract routes of administration."""
        routes = []
        consumed_in_elements = XMLUtils.find_all_elements(manufactured_product_element, "consumedIn")
        
        for consumed_in in consumed_in_elements:
            substance_admin = XMLUtils.find_element(consumed_in, "substanceAdministration")
            if substance_admin is not None:
                route_code_element = XMLUtils.find_element(substance_admin, "routeCode")
